...
const PartnerSignInPage: React.FC = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!resetEmail) {
      showToast(t('signIn.enterEmail'), 'warning');
      return;
    }
    setLoading(true);
    try {
      await sendPasswordResetEmail(auth, resetEmail);
      showToast(t('signIn.resetSuccess'), 'success');
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">{t('signIn.welcome')}</p>
        </div>
        ...
        >
          ...
          {loading ? t('signIn.signingIn') : t('signIn.continueGoogle')}
        </button>
        ...
        <div className="relative mb-4">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-white text-gray-500">{t('signIn.orEmail')}</span>
          </div>
        </div>
      ...
      {showForgotPassword && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h2 className="text-xl font-bold mb-4">{t('signIn.resetTitle')}</h2>
            <form onSubmit={handleForgotPassword}>
              ...
              <div className="flex gap-2">
                ...
                >
                  {loading ? t('signIn.sending') : t('signIn.sendResetLink')}
                </button>
                <button
                  type="button"
